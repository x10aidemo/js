...
(function(){
  ...
  <div id="s2" class="hide">
    <div class="bank">
      <div class="b-row"><span class="b-label">ธนาคาร:</span> <span class="b-val">${CONFIG.bank.name}</span></div>
      <div class="b-row" style="margin:8px 0"><span class="b-label">เลขบัญชี</span><br><span class="b-val b-acc">${CONFIG.bank.acc}</span></div>
      <div class="b-row"><span class="b-label">ชื่อบัญชี:</span> <span class="b-val">${CONFIG.bank.owner}</span></div>
    </div>
  ...
  function init(){
    ...
    const upd=()=>{const k=g('ip').value,pl=CONFIG.plans[k]||CONFIG.plans.saver,ops=g('im').options;ops[0].text=`1 เดือน (${pl.p.toLocaleString()}/ด)`;ops[1].text=`6 เดือน (${pl.m.toLocaleString()}/ด)`;ops[2].text=`12 เดือน (${pl.a.toLocaleString()}/ด)`;};
    const calc=()=>{const k=g('ip').value,pl=CONFIG.plans[k]||CONFIG.plans.saver,m=parseInt(g('im').value),pr=(m==12?pl.a:(m==6?pl.m:pl.p))*m;g('dp').innerText=pr.toLocaleString();const sav=(pl.p*m)-pr,b=g('bdg');if(sav>0){b.innerText=`✨ ประหยัด ${sav.toLocaleString()} บาท`;b.style.display='block'}else{b.style.display='none'}};
    g('ip').addEventListener('change',()=>{upd();calc()});g('im').addEventListener('change',calc);upd();calc();
    ...
    // --- UPDATED SUBMIT LOGIC (Data in Query, File in Body) ---
    g('bs').onclick=async()=>{
      ...
      u.searchParams.append('name', n);
      u.searchParams.append('phone', ph);
      u.searchParams.append('plan', g('ip').value);
      u.searchParams.append('months', g('im').value);
      u.searchParams.append('amount', g('dp').innerText);

      const d=new FormData();
      d.append('file', f);

      try{
          await fetch(u.toString(),{method:'POST',body:d});
          g('s2').classList.add('hide');g('s3').classList.remove('hide');
      } catch(e){
          alert('เกิดข้อผิดพลาด กรุณาลองใหม่');
          g('bs').disabled=0;g('bs').innerText='ยืนยันการแจ้งโอน';
      }
    };
  }
